                           ...
                           int (*okfn)(struct sk_buff*))
{
	...
	printk(KERN_INFO
	       "OUT packet info: src ip: %u, src port: %u; dest ip: %u, dest port: %u; proto: %un", src_ip,
	       src_port, dest_ip, dest_port, ip_header->protocol);
	...
	list_for_each(p, &policy_list.list)
	{
		...
		printk(KERN_INFO
		       "rule %d: a_rule->in_out = %u; a_rule->src_ip = %u; a_rule->src_netmask=%u; a_rule->src_port=%u; a_rule->dest_ip=%u; a_rule->dest_netmask=%u; a_rule->dest_port=%u; a_rule->proto=%u; a_rule->action=%un",
		       i, a_rule->in_out, a_rule->src_ip, a_rule->src_netmask, a_rule->src_port, a_rule->dest_ip,
		       a_rule->dest_netmask, a_rule->dest_port, a_rule->proto, a_rule->action);

		//if a rule doesn't specify as "out", skip it
		if (a_rule->in_out != 2)
		{
			printk(KERN_INFO "rule %d (a_rule->in_out: %u) not match: out packet, rule doesn't specify as outn",
			       i, a_rule->in_out);
			continue;
		}
                          ...
                          int (*okfn)(struct sk_buff*))
{
	...
	printk(KERN_INFO "IN packet info: src ip: %u, src port: %u; dest ip: %u, dest port: %u; proto: %un",
	...
	list_for_each(p, &policy_list.list)
	{
		...
		if (a_rule->in_out != 1)
		{
			printk(KERN_INFO "rule %d (a_rule->in_out:%u) not match: in packet, rule doesn't specify as inn", i,
			       a_rule->in_out);
			continue;
		}
...
void add_a_rule(struct mf_rule_desp* a_rule_desp)
{
	...
	printk(KERN_INFO
	       "add_a_rule: in_out=%u, src_ip=%u, src_netmask=%u, src_port=%u, dest_ip=%u, dest_netmask=%u, dest_port=%u, proto=%u, action=%un",
	       a_rule->in_out, a_rule->src_ip, a_rule->src_netmask, a_rule->src_port, a_rule->dest_ip,
	       a_rule->dest_netmask, a_rule->dest_port, a_rule->proto, a_rule->action);
